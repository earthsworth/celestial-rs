...
#[async_trait]
pub trait FabricApi {
    /// Get the fabric loader versions compatible with the specified game version
    async fn get_fabric_v2_versions_loader_game_version(
        &self,
        arch: String,
        game_version: String,
        ...
        os_release: String,
        hwid: Option<String>,
        hwid_private: Option<String>,
    ) -> ApiResult<Vec<String>>;
}

#[async_trait]
impl FabricApi for RawApiClient {
    async fn get_fabric_v2_versions_loader_game_version(
        &self,
        arch: String,
        game_version: String,
        ...
        let url = self.build_url(&format!(
            "/fabric/v2/versions/loader/{}",
            &game_version.to_string()
        ));
        let mut request = self.client.request(Method::GET, &url);
        let mut query_params = Vec::new();
        if let Some(value) = hwid {
            query_params.push(("hwid".to_string(), value.to_string()));
        }
        if let Some(value) = hwid_private {
            query_params.push(("hwid_private".to_string(), value.to_string()));
        }
